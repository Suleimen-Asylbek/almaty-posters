import Image from "next/image";
import Link from "next/link";
import { Flame } from "lucide-react";
import type { Product } from "@/lib/types";
import { formatPrice } from "@/lib/utils";

interface ProductCardProps {
  product: Product;
  priority?: boolean;
  showNewBadge?: boolean;
}

export function ProductCard({
  product,
  priority = false,
  showNewBadge = false,
}: ProductCardProps) {
  const hasMultipleImages = Array.isArray(product.images) && product.images.length > 1;
  const secondImageUrl = hasMultipleImages ? product.images[1] : null;
  const lowestPrice = product.price_30x40 || 0;

  return (
    <div className="group">
      <Link href={`/product/${product.slug}`} className="block transition-all duration-500 ease-out group-hover:-translate-y-2">
        <div className="relative overflow-hidden rounded-2xl bg-[#F4F4F4] aspect-[3/4] transition-shadow duration-500 group-hover:shadow-xl">
          {/* Images */}
          <div className="relative h-full w-full transition-transform duration-700 ease-out group-hover:scale-105">
            {hasMultipleImages && (
              <Image
                src={secondImageUrl!}
                alt={`${product.title} - ракурс 2`}
                fill
                className="object-cover z-0 opacity-0 transition-opacity duration-500 group-hover:opacity-100"
                sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 25vw"
              />
            )}
            <Image
              src={product.image_url}
              alt={product.title}
              fill
              priority={priority}
              className={`object-cover z-10 transition-opacity duration-500 ${hasMultipleImages ? 'group-hover:opacity-0' : ''}`}
              sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 25vw"
            />
          </div>

          {/* Бейджи */}
          <div className="absolute top-4 left-4 flex flex-col gap-2 z-30">
            {product.featured && (
              <div className="bg-[#F97316] text-white font-bold h-6 min-w-[72px] px-3 rounded-full uppercase tracking-wider flex items-center justify-center gap-1 text-[10px]">
                <Flame size={12} fill="currentColor" />
                Хит
              </div>
            )}
            {showNewBadge && (
              <div className="bg-[#111111] text-white font-bold h-6 px-3 rounded-full uppercase tracking-wider flex items-center justify-center text-[10px]">
                Новинка
              </div>
            )}
            {product.category && (
              <span className="bg-white/90 backdrop-blur-sm text-[#111111] text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wider">
                {product.category.name}
              </span>
            )}
          </div>
        </div>

        {/* Текст */}
        <div className="mt-4 px-1">
          <p className="font-bold text-[#111111] text-sm tracking-tight leading-snug line-clamp-1">
            {product.title}
          </p>
          <p className="text-[#666666] text-sm mt-1 font-medium">
            от {formatPrice(lowestPrice)}
          </p>
        </div>
      </Link>
    </div>
  );
}
